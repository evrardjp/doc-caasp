...
[NOTE]
====
{productname} requires a base set of images to be mirrored, as they contain the core services needed to run the cluster.

The list of images to mirror for {productname} is available from:
...
Refer to: link:https://documentation.suse.com/suse-caasp/4/single-html/caasp-admin/#_configuring_container_registries_for_cri_o[{productname} Admin Guide:Configuring Container Registries for CRI-O]
====
